...
<div class="container-fluid px-0 green-text bg-beige pt-5" style="min-height: 60vh;">
    ...
    <h1 class="fs-1 mb-6 text-center fw-normal-sackers">{{__('Encuentra tu')}}<span class="beige-text"> {{__('Unidad Ideal')}}</span></h1>

    @if ($units->isNotEmpty())

        <div class="row w-100 mx-auto justify-content-center pb-5">

            <h2 class="fs-2 mb-5 text-center fw-normal-sackers">{{__('Resultados')}}</h2>
            @foreach ($units as $unit)
                ...
                <div class="col-11 col-lg-3 mx-2 shadow-7 mb-4 px-0">
                    ...
                    <div class="row w-100 mx-auto fw-normal-zen bg-darkbeige py-4 px-3" style="min-height: 230px;">
                        <div class="col-6">
                            <h2 class="fs-4">{{__('Unidad')}} {{$unit->name}}</h2>
                            ...
                            <h3 class="fs-5">{{__('Modelo')}}: {{$unit->unitType->name}}</h3>
                            <h3 class="fs-5">{{__('Torre')}}: {{$unit->tower->name}}</h3>
                            
                        </div>

                        <div class="col-6">
                            <h2 class="fs-4">${{number_format($unit->price)}} MXN</h2>
                            <h4 class="fs-5 mb-4">{{__('Dimensión')}}: {{$unit->meters_total}} m²</h4>
                            <a href="{{route('view.unit',['id' => $unit->id]);}}" class="btn btn-yellow shadow-7 w-100">{{__('Más Info')}}</a>
                        </div>
                    </div>
    ...
    @else
        ...
        <h3 class="fs-2 pb-5 text-center fw-normal-sackers">{{__('Lo sentimos, no hay resultados.')}}</h3>
